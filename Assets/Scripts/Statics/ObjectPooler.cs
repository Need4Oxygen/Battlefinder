...
public class ObjectPooler : MonoBehaviour
{
    ...
    public static List<GameObject> CreatePool(GameObject prefab, Transform parent, int size)
    {
        if (!Pools.ContainsKey(prefab.name))
        {
            ...
            if (pooleable == null)
            {
                Logger.LogError("ObjPooler", $"Can't make pool for non pooleable object \"{prefab.name}\"");
                return null;
            }
            ...
            Logger.Log("ObjPooler", $"New pool: {prefab.name} with size \"{size}\"");
            return newPool.ToList(); ;
        }
        else
        {
            Logger.LogWarning("ObjPooler", $"Pool with name \"{prefab.name}\" already exist!");
            return null;
        }
    }
